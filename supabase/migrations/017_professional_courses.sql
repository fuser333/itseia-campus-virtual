-- ============================================================
-- ITSEIA Academy — Migration 017: Módulo Cursos Profesionales
-- Fecha: 2 jun 2026
-- Branch: feat/modulo-profesional
-- ============================================================
...
--
-- Tablas creadas:
--   1. professional_courses          — un curso por cohorte
--   2. professional_modules          — agrupación pedagógica (5 módulos)
--   3. professional_sessions         — sesiones en vivo (8 por curso)
--   4. professional_enrollments      — quiénes están inscritos
--   5. professional_session_progress — qué sesiones marcó completas el alumno
--
-- Decisión pendiente para Héctor: ¿reutilizamos ruta /cursos-pro o creamos
--   /profesional? Ver ARQ_PREUNI.md §5 Decisión 1. La migration es agnóstica
--   a esa decisión — funciona en cualquiera de las dos rutas.
-- ============================================================
...
CREATE INDEX IF NOT EXISTS idx_pcourses_active_start
  ON public.professional_courses(is_active, start_date);

COMMENT ON TABLE public.professional_courses IS
  'Cohortes de Cursos Profesionales ITSEIA. Una fila por cohorte. Aislado de programs/courses.';

-- ============================================================
-- 2. professional_modules
-- ============================================================
-- Agrupación pedagógica dentro de un curso (5 módulos para el curso piloto).
-- Una sesión puede pertenecer a un módulo via professional_sessions.module_id.

CREATE TABLE IF NOT EXISTS public.professional_modules (
  id          UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE INDEX IF NOT EXISTS idx_pmodules_course
  ON public.professional_modules(course_id, num);

COMMENT ON TABLE public.professional_modules IS
  'Módulos pedagógicos dentro de un curso profesional. course_id → professional_courses.';

-- ============================================================
-- 3. professional_sessions
-- ============================================================
...
CREATE TABLE IF NOT EXISTS public.professional_sessions (
  id                 UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  course_id          UUID NOT NULL REFERENCES public.professional_courses(id) ON DELETE CASCADE,
  module_id          UUID REFERENCES public.professional_modules(id) ON DELETE SET NULL,
  num                INT NOT NULL,
  title              TEXT NOT NULL,
  description        TEXT,
...
CREATE INDEX IF NOT EXISTS idx_psessions_course_num
  ON public.professional_sessions(course_id, num);
CREATE INDEX IF NOT EXISTS idx_psessions_scheduled
  ON public.professional_sessions(scheduled_at);
CREATE INDEX IF NOT EXISTS idx_psessions_module
  ON public.professional_sessions(module_id);

COMMENT ON TABLE public.professional_sessions IS
  'Sesiones del curso profesional. Una por clase en vivo. 8 pestañas en columnas JSONB.';

-- ============================================================
-- 4. professional_enrollments
-- ============================================================
...
CREATE INDEX IF NOT EXISTS idx_penrollments_profile_status
  ON public.professional_enrollments(profile_id, status);
CREATE INDEX IF NOT EXISTS idx_penrollments_course_status
  ON public.professional_enrollments(course_id, status);

COMMENT ON TABLE public.professional_enrollments IS
  'Inscripciones a cursos profesionales. UNIQUE(course_id, profile_id).';

-- ============================================================
-- 5. professional_session_progress
-- ============================================================
-- Tracking de qué sesiones vio/completó el alumno.

CREATE TABLE IF NOT EXISTS public.professional_session_progress (
  id              UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE INDEX IF NOT EXISTS idx_psprogress_enrollment
  ON public.professional_session_progress(enrollment_id);
CREATE INDEX IF NOT EXISTS idx_psprogress_session
  ON public.professional_session_progress(session_id);

COMMENT ON TABLE public.professional_session_progress IS
...
DROP TRIGGER IF EXISTS trg_pcourses_updated_at ON public.professional_courses;
CREATE TRIGGER trg_pcourses_updated_at
  BEFORE UPDATE ON public.professional_courses
  FOR EACH ROW EXECUTE FUNCTION public.tg_set_updated_at();

DROP TRIGGER IF EXISTS trg_psessions_updated_at ON public.professional_sessions;
CREATE TRIGGER trg_psessions_updated_at
  BEFORE UPDATE ON public.professional_sessions
  FOR EACH ROW EXECUTE FUNCTION public.tg_set_updated_at();

DROP TRIGGER IF EXISTS trg_psprogress_updated_at ON public.professional_session_progress;
CREATE TRIGGER trg_psprogress_updated_at
  BEFORE UPDATE ON public.professional_session_progress
  FOR EACH ROW EXECUTE FUNCTION public.tg_set_updated_at();
...
ALTER TABLE public.professional_session_progress ENABLE ROW LEVEL SECURITY;
...
DROP POLICY IF EXISTS "pcourses_select_active" ON public.professional_courses;
CREATE POLICY "pcourses_select_active"
  ON public.professional_courses FOR SELECT
  TO authenticated
  USING (is_active = true);

DROP POLICY IF EXISTS "pcourses_select_admin" ON public.professional_courses;
CREATE POLICY "pcourses_select_admin"
  ON public.professional_courses FOR SELECT
  TO authenticated
  USING (public.get_user_role() IN ('super_admin','admin','coordinacion'));

DROP POLICY IF EXISTS "pcourses_insert_admin" ON public.professional_courses;
...
DROP POLICY IF EXISTS "pcourses_update_admin" ON public.professional_courses;
CREATE POLICY "pcourses_update_admin"
  ON public.professional_courses FOR UPDATE
  TO authenticated
  USING (public.get_user_role() IN ('super_admin','admin'));

DROP POLICY IF EXISTS "pcourses_delete_admin" ON public.professional_courses;
...
DROP POLICY IF EXISTS "pmodules_select_all" ON public.professional_modules;
CREATE POLICY "pmodules_select_all"
  ON public.professional_modules FOR SELECT
  TO authenticated
  USING (true);

DROP POLICY IF EXISTS "pmodules_write_admin" ON public.professional_modules;
CREATE POLICY "pmodules_write_admin"
  ON public.professional_modules FOR INSERT
  TO authenticated
  WITH CHECK (public.get_user_role() IN ('super_admin','admin'));

DROP POLICY IF EXISTS "pmodules_update_admin" ON public.professional_modules;
CREATE POLICY "pmodules_update_admin"
  ON public.professional_modules FOR UPDATE
  TO authenticated
  USING (public.get_user_role() IN ('super_admin','admin'));

DROP POLICY IF EXISTS "pmodules_delete_admin" ON public.professional_modules;
CREATE POLICY "pmodules_delete_admin"
  ON public.professional_modules FOR DELETE
  TO authenticated
  USING (public.get_user_role() IN ('super_admin','admin'));
...
DROP POLICY IF EXISTS "psessions_select_enrolled" ON public.professional_sessions;
CREATE POLICY "psessions_select_enrolled"
  ...
  USING (
    EXISTS (
      ...
      WHERE e.course_id = professional_sessions.course_id
        AND e.profile_id = auth.uid()
        AND e.status = 'active'
    )
  );

DROP POLICY IF EXISTS "psessions_select_staff" ON public.professional_sessions;
CREATE POLICY "psessions_select_staff"
  ON public.professional_sessions FOR SELECT
  TO authenticated
  USING (public.get_user_role() IN ('super_admin','admin','coordinacion','docente'));

DROP POLICY IF EXISTS "psessions_insert_admin" ON public.professional_sessions;
CREATE POLICY "psessions_insert_admin"
  ON public.professional_sessions FOR INSERT
  TO authenticated
  WITH CHECK (public.get_user_role() IN ('super_admin','admin'));

DROP POLICY IF EXISTS "psessions_update_admin" ON public.professional_sessions;
CREATE POLICY "psessions_update_admin"
  ON public.professional_sessions FOR UPDATE
  TO authenticated
  USING (public.get_user_role() IN ('super_admin','admin','coordinacion'));

DROP POLICY IF EXISTS "psessions_delete_admin" ON public.professional_sessions;
CREATE POLICY "psessions_delete_admin"
  ON public.professional_sessions FOR DELETE
  TO authenticated
  USING (public.get_user_role() IN ('super_admin','admin'));

-- ─── professional_enrollments ───────────────────────────────
-- Lectura:
...
DROP POLICY IF EXISTS "penrollments_select_own" ON public.professional_enrollments;
CREATE POLICY "penrollments_select_own"
  ON public.professional_enrollments FOR SELECT
  TO authenticated
  USING (profile_id = auth.uid());

DROP POLICY IF EXISTS "penrollments_select_staff" ON public.professional_enrollments;
CREATE POLICY "penrollments_select_staff"
  ON public.professional_enrollments FOR SELECT
  TO authenticated
  USING (public.get_user_role() IN ('super_admin','admin','coordinacion','docente'));

DROP POLICY IF EXISTS "penrollments_insert_admin" ON public.professional_enrollments;
CREATE POLICY "penrollments_insert_admin"
  ON public.professional_enrollments FOR INSERT
  TO authenticated
  WITH CHECK (public.get_user_role() IN ('super_admin','admin'));

DROP POLICY IF EXISTS "penrollments_update_admin" ON public.professional_enrollments;
CREATE POLICY "penrollments_update_admin"
  ON public.professional_enrollments FOR UPDATE
  TO authenticated
  USING (public.get_user_role() IN ('super_admin','admin','coordinacion'));

DROP POLICY IF EXISTS "penrollments_delete_admin" ON public.professional_enrollments;
CREATE POLICY "penrollments_delete_admin"
  ON public.professional_enrollments FOR DELETE
  TO authenticated
  USING (public.get_user_role() = 'super_admin');

-- ─── professional_session_progress ──────────────────────────
-- Lectura:
...
DROP POLICY IF EXISTS "psprogress_select_own" ON public.professional_session_progress;
CREATE POLICY "psprogress_select_own"
  ON public.professional_session_progress FOR SELECT
  TO authenticated
  USING (
    EXISTS (
      SELECT 1 FROM public.professional_enrollments e
      WHERE e.id = professional_session_progress.enrollment_id
        AND e.profile_id = auth.uid()
    )
  );

DROP POLICY IF EXISTS "psprogress_select_staff" ON public.professional_session_progress;
CREATE POLICY "psprogress_select_staff"
  ON public.professional_session_progress FOR SELECT
  TO authenticated
  USING (public.get_user_role() IN ('super_admin','admin','coordinacion','docente'));

DROP POLICY IF EXISTS "psprogress_insert_own" ON public.professional_session_progress;
CREATE POLICY "psprogress_insert_own"
  ON public.professional_session_progress FOR INSERT
  TO authenticated
  WITH CHECK (
    EXISTS (
      SELECT 1 FROM public.professional_enrollments e
...
DROP POLICY IF EXISTS "psprogress_update_own" ON public.professional_session_progress;
CREATE POLICY "psprogress_update_own"
  ON public.professional_session_progress FOR UPDATE
  TO authenticated
  USING (
    EXISTS (
      SELECT 1 FROM public.professional_enrollments e
...
DROP POLICY IF EXISTS "psprogress_admin_all" ON public.professional_session_progress;
CREATE POLICY "psprogress_admin_all"
  ON public.professional_session_progress FOR ALL
  TO authenticated
  USING (public.get_user_role() IN ('super_admin','admin','coordinacion'))
...
--    WHERE schemaname='public' AND tablename LIKE 'professional_%'
--    ORDER BY tablename;
--    -- Esperado: 5 filas.
--
-- 2. RLS habilitado:
...
--    SELECT count(*) FROM public.professional_courses WHERE is_active = true;
--    -- Debe responder sin "infinite recursion".
--
-- 5. Test enrollment: como alumno autenticado SIN enrollment:
--    SELECT count(*) FROM public.professional_sessions;
--    -- Debe devolver 0.
-- ============================================================
