...
const WA_INFO = "https://wa.me/593990709009?text=Quiero%20m%C3%A1s%20informaci%C3%B3n%20sobre%20los%20Cursos%20Profesionales%20de%20ITSEIA";
...
const BENEFICIOS = [
  "Diagnóstico gratuito de tu perfil profesional",
  "Contenido 100% a la medida de tu industria",
  "Casos reales de tu sector específico",
  "Proyecto práctico que aplicas desde el día 1",
  "Asesoría 1:1 con Héctor Velasco",
  "Sin prerequisitos técnicos — desde cero",
];
          ...
          <div>
            <div className="rounded-2xl border border-white/[0.08] bg-white/[0.03] p-7 mb-6">
              <p className="text-white/70 leading-relaxed text-base mb-6">
                Héctor evalúa tu perfil profesional y diseña un curso con casos reales
                de tu industria. Nada genérico: cada módulo aplica directamente a lo que
                ya haces. Desde la primera sesión empiezas a aplicar en tu trabajo real.
              </p>
            ...
            {/* Pricing card */}
            <div
              className="rounded-2xl border border-[#F0846D]/30 p-7 text-center"
              style={{
                background:
                  "linear-gradient(145deg, rgba(240,132,109,0.07) 0%, rgba(31,47,88,0.25) 100%)",
                backdropFilter: "blur(12px)",
              }}
            >
              <div className="inline-flex items-center gap-2 bg-[#F0846D]/10 border border-[#F0846D]/20 rounded-full px-3 py-1.5 mb-4">
                <span className="text-[#F0846D] text-xs font-bold uppercase tracking-wider">
                  Planes disponibles
                </span>
              </div>
              <div className="grid grid-cols-3 gap-3 mb-5">
                {PLANES.map((p) => (
                  <div key={p.nombre} className="text-center">
                    <div className="text-2xl font-extrabold text-[#F0846D]" style={{ fontFamily: "var(--font-space-grotesk)" }}>{p.precio}</div>
                    <div className="text-white/40 text-xs">{p.horas}</div>
                    <div className="text-[#F0846D] text-xs font-bold uppercase tracking-wider mt-0.5">{p.nombre}</div>
                  </div>
                ))}
              </div>
