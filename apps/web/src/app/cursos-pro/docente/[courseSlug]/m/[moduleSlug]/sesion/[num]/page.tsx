...
/**
 * Sesión de Cursos Profesionales · VISTA DOCENTE (Libro del Profesor)
 * URL: /cursos-pro/docente/[courseSlug]/m/[moduleSlug]/sesion/[num]
 *
 * Espejo de la vista alumno + 4 pestañas extra para docente:
 *  10. Asistencia · 11. Subir Grabación · 12. Stats Quiz · 13. Feedback
 *
 * Guard: solo role docente/admin/coordinacion/super_admin/finanzas.
 * NO requiere enrollment activo (docente puede no estar matriculado).
 * NO toca /teacher/ (LIVE para preuni).
 */
...
// Paneles docente (LIBRO PROFESOR)
import TeacherAttendancePanel from "@/components/session/teacher/TeacherAttendancePanel";
import TeacherUploadRecordingPanel from "@/components/session/teacher/TeacherUploadRecordingPanel";
import TeacherQuizStatsPanel from "@/components/session/teacher/TeacherQuizStatsPanel";
import TeacherFeedbackPanel from "@/components/session/teacher/TeacherFeedbackPanel";
...
export default function CursoProSessionPage({ params }: PageProps) {
  ...
  const loadData = useCallback(async () => {
    ...
    // ─── Guard VISTA DOCENTE: requiere role staff (NO matrícula) ──────
    // A diferencia de la vista alumno, la vista docente requiere SOLO
    // role staff (docente / admin / coordinacion / super_admin / finanzas).
    // No exige matrícula porque el docente generalmente no la tiene.
  ...
  // ─── Acceso denegado (no es staff) ────────────────────────────────
  if (accessDenied) {
    return (
      <div className="flex h-[calc(100vh-64px)] items-center justify-center">
        <div className="text-center max-w-md px-6">
          <GraduationCap className="mx-auto size-12 text-[#F0846D]" />
          <p className="mt-3 text-base font-semibold text-white">
            Vista docente restringida
          </p>
          <p className="mt-1 text-sm text-white/60">
            Esta es la vista del profesor. Requiere cuenta con role docente,
            admin, coordinación o staff. Si eres alumno, usa la vista del curso
            normal.
          </p>
          <Link
            href={`/cursos-pro/c/${courseSlug}/m/${moduleSlug}/sesion/${num}`}
  ...
  const tabs: SessionTab[] = [
    ...
    // ─── Pestañas exclusivas del docente (LIBRO PROFESOR) ────────────
    {
      id: "attendance",
      label: "Asistencia",
      icon: "live" as SessionTab["icon"],
    ...
    },
    {
      ...
      available: true,
      content: (
        <TeacherUploadRecordingPanel
          sessionId={session.id}
          initialUrl={session.recording_url}
        />
      ),
    },
    {
      id: "quiz-stats",
      label: "Stats Quiz",
      icon: "quiz" as SessionTab["icon"],
      completed: false,
      available: true,
      content: (
        <TeacherQuizStatsPanel
          questions={(quizQuestions as InlineQuizQuestion[] | null) || null}
        />
      ),
    },
    {
      id: "teacher-feedback",
      label: "Notas docente",
      icon: "theory" as SessionTab["icon"],
      completed: false,
      available: true,
      content: <TeacherFeedbackPanel sessionId={session.id} />,
    },
  ];

  const moduleUrl = `/cursos-pro/docente/${courseSlug}/m/${moduleSlug}`;
  const currentNumInModule = parseInt(num);

  return (
    <div className="flex h-screen flex-col">
      {/* Top bar — VISTA DOCENTE (acento gold + badge) */}
      <header className="border-b border-[#FBBC0C]/30 bg-[#0A1628] px-4 py-2 shadow-sm">
        <div className="hidden md:flex items-center gap-1 mb-1">
          <Breadcrumb
            items={[
              { label: "Panel docente", href: "/cursos-pro/docente" },
              { label: course.name, href: `/cursos-pro/docente/${courseSlug}` },
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2 max-w-[70%]">
            <span className="inline-flex items-center gap-1 rounded-full bg-[#FBBC0C]/20 border border-[#FBBC0C]/40 px-2 py-0.5 text-[9px] font-bold uppercase tracking-widest text-[#FBBC0C] shrink-0">
          ...
          <p className="text-[10px] text-white/50 flex-shrink-0">
        ...
        prevSession={
          prevSession
            ? {
                ...
                url: `/cursos-pro/docente/${courseSlug}/m/${moduleSlug}/sesion/${prevSession.numInModule}`,
              }
            : null
        }
        nextSession={
