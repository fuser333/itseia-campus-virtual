...
import { BookOpen, Play, CheckCircle2, Clock, ChevronRight, Brain } from "lucide-react";

const WEEK_NUMBER = 1;

export default async function PreuniSemana1() {
  ...
  // 2. Fetch the semester that corresponds to this week (number = WEEK_NUMBER)
  let semester = null;
  if (program) {
  ...
  // 4. Fetch sessions for each subject (all in one query using IN)
        ...
        <div className="flex items-start gap-4 mt-3">
          ...
          <div className="flex-1">
            <h1
              ...
              style={{ color: "#0A1628" }}
            >
              Semana {WEEK_NUMBER}: {semester.name || "Fundamentos de IA"}
            </h1>
            <p className="mt-1 text-sm" style={{ color: "#1F2F58AA" }}>
              {subjects.length} materia{subjects.length !== 1 ? "s" : ""} &middot; {totalSessions} sesion{totalSessions !== 1 ? "es" : ""}
            </p>
          </div>
        </div>
