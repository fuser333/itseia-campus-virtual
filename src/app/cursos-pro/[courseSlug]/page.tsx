/**
 * Página de curso · /cursos-pro/[courseSlug]
 *
 * Maneja dos casos:
 *  A) Curso en BD (ej: admin-salud) → lista módulos con sesiones (Server Component)
 *  B) Curso legacy con data estática (ej: steveen-pinchao) → vista antigua
 *
 * La lógica: primero intenta buscar el curso en cursos_pro_courses.
 * Si existe → renderiza la vista BD.
 * Si no existe → verifica si es un slug legacy y renderiza la vista legacy.
 */
...
export default async function CursoProDetailPage({ params }: PageProps) {
  ...
  if (!user) redirect(`/login?module=cursos-pro&next=/cursos-pro/${courseSlug}`);

  const course = await getCourseBySlug(courseSlug);

  // Si no hay curso en BD, notFound (los slugs legacy apuntan a /cursos-pro/c/ o directamente)
  ...
  return (
    <div className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8 py-8 space-y-8">
      ...
      <div>
        <Link
          href="/cursos-pro"
          className="inline-flex items-center gap-1.5 text-xs font-medium text-white/50 hover:text-white mb-3 transition-colors"
        >
        ...
        <div className="space-y-6">
          {modules.map((m) => {
            ...
            return (
              <div
                key={m.id}
                className="rounded-2xl border border-white/8 bg-[#1F2F58]/15 overflow-hidden"
              >
                <div className="bg-[#73B8E7]/8 px-5 py-3 border-b border-white/8">
                ...
                <ul className="divide-y divide-white/[0.06]">
                  {moduleSessions.map((s) => {
                    ...
                    return (
                      <li key={s.id}>
                        ...
                        >
                          <span className="flex size-8 shrink-0 items-center justify-center rounded-full bg-white/8 text-[11px] font-bold text-white/60">
                            S{numInMod}
                          </span>
                          <div className="flex-1 min-w-0">
                            ...
                            <p className="text-[11px] text-white/45 mt-0.5 flex items-center gap-3">
                  ...
                  {moduleSessions.length === 0 && (
                    <li className="px-5 py-4 text-[11px] text-white/35 italic">
        ...
        <section className="rounded-2xl border-2 border-dashed border-[#FBBC0C]/40 bg-[#FBBC0C]/5 p-5">
          <div className="flex items-center justify-between gap-4">
            ...
            <Link
              ...
              className="inline-flex items-center gap-2 rounded-lg bg-[#0A1628] px-4 py-2.5 text-sm font-bold text-white hover:bg-[#1F2F58] transition-colors border border-white/10"
            >
              Panel docente
